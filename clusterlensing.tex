...
We can rewrite the differential surface mass density in the form in which it is computed in the \code{nfw} module:
\begin{equation}
...
The function \code{calc_delta_c()} takes a single input parameter, the cluster concentration \code{c200} (e.g. as calculated by one of the functions in the \code{cofm} module), and returns the characteristic halo overdensity:
\begin{equation}
