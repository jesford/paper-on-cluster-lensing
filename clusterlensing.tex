...
\end{tabular}
\caption{\bf Total runtime for calculating both the $\Sigma(R)$ and $\Delta\Sigma(R)$ NFW profiles for different numbers of clusters. The calculation time is constant for centered calculations. For miscentered calculations, the runtime scales linearly with the number of clusters and with the number of radial bins (these examples all assume 10 bins).}
\label{runtimetable}
\end{table}
...
{\bf With the help of \code{emcee} \citep{emcee}, we use 20 walkers to draw 500 samples each from the posterior. This sampling process took ??? seconds to run, which is significant because of the miscentering integrations (as shown in Table \ref{runtimetable}, the calculations are many times faster for the centered cluster case).}
...
{\bf Finally, we use the \code{corner} package to neatly display the results...}
...
\software{NumPy \citep{NumPy}, SciPy \citep{SciPy}, Pandas \citep{Pandas}, matplotlib \citep{matplotlib}, IPython \citep{IPython}, AstroPy \citep{astropy13}, Seaborn \citep{Seaborn}.}
