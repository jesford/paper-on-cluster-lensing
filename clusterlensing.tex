...
Another related set of code is provided by J\"{o}rg Dietrich's NFW routines \attn{insert bibtex for Jorg's code}, available on GitHub at \url{https://github.com/joergdietrich/NFW}. These \code{Python} routines calculate NFW profiles for $\Sigma(R)$ and $\Delta\Sigma(R)$, and \attn{invoke mass-concentration relations including \citet{Duffy08} and another one (an incomplete overlap with \code{cluster-lensing}). What else exactly does this code do?}. Similarly, \code{cluster-lensing} goes beyond the functionality of \attn{insert bibtex for Jorg's code} by supplying means for calculating cluster miscentering.
